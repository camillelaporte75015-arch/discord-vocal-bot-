...
client.on('messageCreate', async (message) => {
  ...
  if (cmd === 'help') {
    const embed = new EmbedBuilder()
      .setColor(0x5865F2)
      .setTitle('Commandes du bot vocal')
      .setDescription('Prefix : `=`')
      .addFields(
        {
          name: 'Commandes vocales — WL / OW requis',
          value: [
            '`=mv <@user|id>` — Deplace quelqun dans votre vocal',
            '`=find <@user|id>` — Trouve dans quel vocal est quelqun',
            '`=mvall <#salon|id>` — Deplace tout le monde de votre vocal vers un autre (cooldown 2min)',
            '`=wakeup <@user|id>` — Deplace quelqun dans tous les vocaux pendant 30s (cooldown 5min/cible)',
            '`=join <@user|id>` — Vous rejoignez le vocal de quelqun',
            '`=deco <@user|id>` — Deconnecte quelqun dun vocal (cooldown 1min30)',
          ].join('\n'),
        },
        {
          name: 'Commandes whitelist — OW requis',
          value: [
            '`=wl <@user|id>` — Ajoute quelqun a la whitelist',
            '`=unwl <@user|id>` — Retire quelqun de la whitelist',
            '`=wl list` — Affiche la liste des OW et WL',
          ].join('\n'),
        },
        {
          name: 'Commandes owner — Super Owner uniquement',
          value: [
            '`=ow <@user|id>` — Donne le statut owner du bot',
            '`=unow <@user|id>` — Retire le statut owner du bot',
            '`=play <activite>` — Change lactivite du bot',
          ].join('\n'),
        },
        {
          name: 'Niveaux de permission',
          value: [
            '**Super Owner** — Acces total, non revocable',
            '**Owner (OW)** — Commandes vocales + gestion de la WL',
            '**Whitelist (WL)** — Commandes vocales uniquement',
          ].join('\n'),
        },
      )
      .setFooter({ text: 'Les cooldowns sont indiques entre parentheses.' })
      .setTimestamp();
  ...
  if (!['mv', 'find', 'mvall', 'wakeup', 'join', 'deco'].includes(cmd)) return;
